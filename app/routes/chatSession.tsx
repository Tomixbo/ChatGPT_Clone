...
export async function action({ params, request }: Route.ActionArgs) {
  // Cette action n'est plus utilisée avec la gestion du stream dans ChatSession
...
export default function ChatSession({ loaderData }: Route.ComponentProps) {
  ...
  const handleSubmit = async (userMessage: string) => {
    // 1. Affichage optimiste du message user
    ...
    try {
      // 2. Envoi de la requête PUT vers le serveur
      const response = await fetch(
        `http://localhost:3000/api/session-chats/${sessionChat.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.body) {
        throw new Error("Le body de la réponse est nul");
      }

      // 3. Assurer un message assistant pour l'affichage incrémental
      setLocalChatHistory((prev) => {
      ...
      // 4. Lecture du flux en style SSE / OpenAI
      ...
      while (!done) {
        ...
        // Décoder le morceau reçu, ajouter au buffer
        const chunk = decoder.decode(value, { stream: true });
        buffer += chunk;

        // Séparer par lignes
        const lines = buffer.split("\n");
        // Garder la dernière ligne incomplète dans buffer
        buffer = lines.pop() || "";
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith("data: ")) {
            ...
            try {
              ...
              // Chemin style GPT : parsed.choices[0].delta.content
              const token = parsed?.choices?.[0]?.delta?.content || "";
              console.log("Token reçu :", token);

              // 5. Mettre à jour le dernier message assistant
              setLocalChatHistory((prev) => {
                const updated = [...prev];
              ...
              console.error("Erreur de parsing JSON pour :", jsonStr, err);
            }
          }
        }
      }
  ...
  const handleCancel = () => {
    console.log("Annulation du stream (handleCancel)");
    // Vous pouvez ajouter ici la logique d'annulation avec AbortController si nécessaire.
  };
